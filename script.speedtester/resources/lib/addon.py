...
from kodiutils import addon_path, get_addon_info, get_setting, get_setting_bool, get_setting_int, localize, log, log_error
...
class SpeedTest(Animation):
    ...
    def speedtest(self, share=False, simple=False, src=None, timeout=10):
        ...
        if get_setting_bool('upload_test', True):
            ...
            self.update_textbox(start_st)
            ulspeed = 0
            prefer_https = get_setting_bool('prefer_https', True)
            for upload_url in upload_urls_for_server(best, prefer_https=prefer_https):

...
### v1.1.5 (2026-08-30)
- Unofficial update by The Red Wizard
- Keeps local results if there is no speedtest.net image
- Upload tries HTTPS first, then HTTP, instead of failing immediately
- Shows a toast if the test fails
- Ping uses a 10 second timeout and averages the probes that actually ran

### v1.1.4 (2026-08-28)
- Unofficial update by The Red Wizard
- Modern Ookla server discovery (api/js/servers) replacing the deprecated servers-static.php list
- Automatic nearest-server selection worldwide, plus optional city/country search or specific server ID
- Prefer-HTTPS server option and configurable connection timeout
- Optional download-only mode and optional result submission, off by default
- Settings and extra translations from script.speedtester pull request 31
- Python 3.9-3.12 compatibility (threading is_set) for Kodi 19-21
